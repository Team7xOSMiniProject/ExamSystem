...
void Server::handleStudentExamRequest(int sock, ExamManager exam) {
    ...
    string response(buffer);
        ...
        if (response == "y" || response == "Y") {
            ...
            // Receive exam type from client: "s" = single, "m" = multiple
            char typeBuf[32] = {0};
            ...
            if (examTypeBuffer == "s") {
                ...
                // If already attempted, notify client and abort
                if (alreadyAttempted) {
                    char message = 'y';
...
void Server::sendAvailableExams(int sock, const string& username, vector<string>& examNames) {
    ...
    for (const auto& exam : exams) {
        ...
        if (include_exam) {
            examNames.push_back(exam_name);  // Store the name to map with exam number
    ...
    all_exams += "\0";  // Null-terminate the string
    ...
    if (user_type == "student") {
        ...
        while (true){
            ...
            if (request == "1") {
                ...
                // Step 3: Validate selection and trigger analysis
