...
void Server::handleViewPerformance(int clientSock, const string& studentId) {
    ...
    // If no attempts file exists, notify client and return
    if (!file.is_open()) {
    ...
    while (true) {
        ...
        if (examChoice == 0) break; // Go back to main menu

        // Validate selection
        if (examChoice < 1 || examChoice > (int)examNames.size()) {
            string mesg = "[!] Invalid option! please select a valid exam.";
            send(clientSock, mesg.c_str(), mesg.size(), 0);
            usleep(1200000);  // Pause for 1.2 seconds
            continue;
        }
        ...
        if (attemptChoice == 0) continue; // Back to exam list

        if (attemptChoice < 1 || attemptChoice > (int)attempts.size()) {
        ...
        perfFilePath = perfFilePath.substr(perfFilePath.find('|') + 1); // Extract actual perf file path
        ...
        while (getline(perfFile, line)) {
            if (line == "START") {
                ...
                for (const auto& exam : exams) {
                    ...
                    if (include_exam) {
                        if (exam_type == "q") break; // skip if practice exam
                        tm tm = {};
                ...
                // Format detailed attempt summary to send to client
                formatted = "\n========== Attempt Details ==========\n\n";
                ...
                while (getline(perfFile, line) && line != "END") {
                    // Process each question attempt line
                    int qNum = 1;
                    while (getline(perfFile, line)) {
                        if (line == "START") break;
                        ...
                        correct = (qNum - 1 < (int)answers.size()) ? answers[qNum - 1] : "?";
        ...
        // Wait for client to request leaderboard info (1) or not (0)
        ...
        // Show leaderboard for this exam
        analyzeExam(selectedExam, clientSock, true);
        break;
    }
}

// Sends the list of exams uploaded by the specified instructor (username) to the client connected via sock.
// It filters the global exams list to include only exams where the instructor matches the username,
// formats each exam's details (excluding the instructor line and replacing exam type codes with descriptions),
// compiles a numbered list of these exams, stores their names in examNames vector,
// and sends the formatted exam list back to the client. If no exams are found for the instructor,
// it sends a message indicating no exams have been uploaded.
        ...
        while (getline(iss, line)) {
            if (line.find("Instructor:") != string::npos) {
                ...
                if (instructor_name == username) {
                    ...
                    break; // Not the current instructor
                }
            }
            ...
            if (line.find("Instructor:") != string::npos) continue;

            filteredExam << line << " | ";
        }

        if (include_exam) {
            string formatted = filteredExam.str();
            if (!formatted.empty() && formatted.size() >= 3 && formatted.substr(formatted.size() - 3) == " | ") {
                formatted.erase(formatted.size() - 3);
            }

            examNames.push_back(exam_name);
            all_exams += to_string(qno++) + ". " + formatted + "\n";
        }
    }
...
void* Server::handle_client(void* client_socket) {
    ...
    while (true) {
        ...
        // If authentication is successful, map socket to username
        if (authenticated) {
    ...
    // --- Post-authentication: Handle role-based logic ---
    ...
    if (user_type == "student") {
        ...
        // Receive exam name (if resuming or submitting pending answers)
        ...
        // If not "n" (no exam), save student answers
        if (examName != "n") {
            receiveStudentAnswers(sock, examName);
        }
        ...
        while (true) {
            ...
            if (request == "1") {
                ...
                if (exam_type == "g" || exam_type == "G") {
                    ...
                    if (ss.fail()) {
                        cerr << "Invalid format. Please use YYYY-MM-DD HH:MM:SS\n";
